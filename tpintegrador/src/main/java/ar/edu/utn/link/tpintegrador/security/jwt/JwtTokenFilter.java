...
public class JwtTokenFilter  extends OncePerRequestFilter {
	   ...
	   private final static Logger logger = LoggerFactory.getLogger(JwtTokenFilter.class);
	    ...
	    protected void doFilterInternal(HttpServletRequest req, HttpServletResponse res, FilterChain filterChain) throws ServletException, IOException {
	        try {
	            String token = getToken(req); //obtiene el token de la peticion
	            if(token != null && jwtProvider.validateToken(token)){
	                String nombreUsuario = jwtProvider.getNombreUsuarioFromToken(token); //trae el nombre del usuario con ese token
	        ...
	        filterChain.doFilter(req, res);//y esto averiguar
	    }
